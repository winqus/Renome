...
function NavButton(element, activeStateClasses, navManager, targetMenu) {
  this.element = element;
  this.activeStateClasses = activeStateClasses.split(" ");
  this.targetMenu = targetMenu;

  this.Toggle = () => {
    if(targetMenu.IsOpen()) {
      navManager.HideNavMenusOrExceptOne();
    } else {
      if(this.element.id != mainNavButtonId || navManager.IsAnyNavOpen() == false)
      {
        targetMenu.Show();
      }
      navManager.HideNavMenusOrExceptOne(targetMenu);
    }
  };
